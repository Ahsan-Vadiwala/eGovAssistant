...
def tokenize(text: str) -> list[str]:
    ...
    Keeps words and numbers while removing punctuation.
    """
...
class BM25Retriever:
    ...
    def __init__(
        ...
        if not self.metadata:
            ...
            raise RuntimeError(
                "Metadata contains no chunks."
            )


        print(
            f"Preparing BM25 corpus: "
    ...
    def rank_candidates(
        ...
        """
        Rank an externally supplied candidate pool using BM25.

        IMPORTANT:

        This is the method used by the web-grounded pipeline.

        The candidates are NOT restricted to section_metadata.json.
        ...
        temporary_bm25 = BM25Okapi(
            tokenized_candidates
        )

        scores = temporary_bm25.get_scores(
            query_tokens
        )
